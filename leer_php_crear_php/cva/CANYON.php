<?php
	...
	$brand_id = 138;
	$result_new = 0;
	$sql = "SELECT product_clave_cva FROM products WHERE product_clave_cva <> '' AND product_brand = 138 ORDER BY product_clave_cva ASC";
	$catalogo_cva_sql = mysql_query($sql);
	while($row = mysql_fetch_array($catalogo_cva_sql)){
		$catalogo_cva[] = $row['product_clave_cva'];
	}
	
	$sql_new = "SELECT product_clave_cva FROM new_products WHERE product_clave_cva <> '' AND brand_id= 138 ORDER BY product_clave_cva ASC";
	...
	$sXML = download_page('http://www.grupocva.com/catalogo_clientes_xml/lista_precios.xml?cliente=26683&marca=CANYON&grupo%&clave=%&codigo=%&sucursales=1&tc=1&tipo=1&depto=1&dt=1&dc=1&promos=1');
	$product_n=0;
	if($sXML){
