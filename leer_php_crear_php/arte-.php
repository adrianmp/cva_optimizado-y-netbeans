<?php
	...
	class Update 
	{
		...
		function products()
		{
			$sql = "SELECT product_clave_cva, product_category FROM products WHERE product_clave_cva <> '' ORDER BY product_clave_cva ASC";
			...
			for ($i=0; $i < $size; $i++) 
			{
				...
				if(!in_array($this->catalogo_cva[$i], $this->catalogo_new))
				{
					...
					if($temp==TRUE)
					{
						...
						echo $query ="DELETE FROM products WHERE product_clave_cva = '".$this->catalogo_cva[$i]."'";
						mysql_query($query);
					}
				}
			}
					...
					if ($num) 
					{
						if(in_array($item->clave, $this->catalogo_cva))
						{
							...
							if($item->moneda =="Dolares")
							{
								...
								$p = (float) $this->google($dolar);
								$product_price_mx = $this->load_price($this->product_category[$pos], $p);	
								$sql = "UPDATE products SET product_buy_mx = " . $p  . " , product_status = 1, product_price_mx =".$product_price_mx;
								$dolares = TRUE;
							}
							else 
							{
								$product_price_mx  = $this->load_price($this->product_category[$pos], $item->precio);
								$sql = "UPDATE products SET product_buy_mx = " . $item->precio  . " , product_status = 1,  product_price_mx =".$product_price_mx;
								$dolares = FALSE;
							}
							if($item->PrecioDescuento!="Sin Descuento")
							{
								...
								if($dolares == TRUE)
								{
									$product_buy_mx_offer = $this->load_price($this->product_category[$pos], $this->google($item->PrecioDescuento));
									$pieces = explode("/", $item->VencimientoPromocion);
									$VencimientoPromocion = $pieces[2]."-".$pieces[1]."-".$pieces[0];
									$sql = "UPDATE products SET product_buy_mx = " . $p  . " , product_status = 1,  product_buy_mx_offer =".$this->google($item->PrecioDescuento)." , product_price_mx =".$product_buy_mx_offer." , product_featured_end ='".$VencimientoPromocion."19:00:00'";
								}
								else
								{
									...
									$pieces = explode("/", $item->VencimientoPromocion);
									$VencimientoPromocion = $pieces[2]."-".$pieces[1]."-".$pieces[0];
									$sql = "UPDATE products SET product_buy_mx = " . $item->precio  . " ,  product_status = 1,  product_buy_mx_offer =".$item->PrecioDescuento." ,  product_price_mx =".$product_buy_mx_offer." , product_featured_end ='".$VencimientoPromocion."19:00:00'";
								}
							}
							if($disponible==0)
							{
